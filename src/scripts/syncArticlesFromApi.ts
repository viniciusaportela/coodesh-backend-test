...
export async function syncArticlesFromApi() {
  try {
    ...
    const response = await fetch(`${config.syncFetchApi}/articles/count`);
    ...
    for (let page = 0; page < articlesCount / SYNC_ARTICLES_PER_PAGE; page += 1) {
      ...
      const response = await fetch(`${config.syncFetchApi}/articles?_sort=id&_limit=${SYNC_ARTICLES_PER_PAGE}&_start=${page * SYNC_ARTICLES_PER_PAGE}`);
      const articles = await response.json() as IArticle[];

      const articlesInDb = await ArticleService.hasMany(articles.map(article => article.id));
      const articlesInDbIdOnly = articlesInDb.map(article => article.id);
      const articlesToAdd = articles.filter(article => !articlesInDbIdOnly.includes(article.id));
